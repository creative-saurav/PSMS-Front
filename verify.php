...
if(isset($_POST['st_mobile_send_btn'])){
    ...
    $smsresult = file_get_contents("http://66.45.237.70/api.php?username=XXXXXXXXXXX&password=XXXXXXXXX&number=88".$user_mobile."&message=$message");

        $stm = $pdo->prepare("UPDATE students SET mobile_code=? WHERE id=?");
        $stm->execute(array($code,$user_id));

        $_SESSION['mobile_code_send']=1;
        $success ="Code Send Success ,Please check your Mobile!";
}
    ...
    if(isset($_POST['st_mobile_verify_btn'])){
        ...
        if(empty($st_code)){
            ...
            $success ="Your mobile verified Succcess!";
        }
    }
		...
		<div class="account-form-inner">
			<div class="account-container">
				<div class="heading-bx left">
					...
					<p><u><?php echo Student('name',$_SESSION['st_loggedin'][0]['id']); ?>'s</u> Please verify your Acount.</p>
				</div>	
                        ...
                        <?php if(isset($_SESSION['email_code_send']) == 1):?>
                          ...
                          <?php endif;?>
                           ...
                           <form class="contact-bx" method="POST" action="">
                                <div class="row placeani">
                                    <div class="col-lg-12 m-b30">
                                        <button name="st_email_send_btn" type="submit" value="Submit" class="btn button-md">Click to Verify Email</button>
                                    </div>
                                </div>
                            </form>
                         <?php endif;?>
                         ...
                         <?php if($mobile_status !=1 AND !isset($_SESSION['mobile_code_send'])):?>     
                           <form class="contact-bx" method="POST" action="">
                                <div class="row placeani">
                                    <div class="col-lg-12 m-b30">
                                        <button name="st_mobile_send_btn" type="submit" value="Submit" class="btn button-md">Click to Verify Mobile</button>
                                    </div>
                                </div>
                            </form>
                         <?php endif;?>
